...
struct EpisodesUseCaseTests {
    ...
    func fetchForwards() async throws {
        let repository = StubEpisodesRepository(episodes: [.make(name: "Pilot")])
    ...
    func fetchRethrows() async {
        let useCase = EpisodesUseCase(repository: StubEpisodesRepository(error: TestError()))
    ...
    func linksAreJoinedByNumber() async throws {
        let repository = StubEpisodesRepository(
    ...
    func emptyLinksLeaveEveryEpisodeUnlinked() async throws {
        let repository = StubEpisodesRepository(episodes: [.make(name: "Pilot")], links: .empty)
