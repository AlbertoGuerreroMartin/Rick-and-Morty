...
///
/// **This is the Characters copy of the Episodes feature's query, and the
/// duplication is deliberate.** Feature packages are independent by design —
/// `Characters` cannot `import Episodes` without turning two modules that have
/// no reason to change together into one — and the alternative, promoting the
/// JustWatch pipeline into a shared package, would put a third party's
/// unofficial endpoint in the same layer as `Networking` and `Storage` to save
/// four files. The document text is kept **byte-for-byte identical** to the
/// Episodes one on purpose: the cache identifier is the root field plus a hash
/// of the document plus the variables, so an identical document means an
/// identical key — the two features address the same lookup, each under its own
/// namespace, and neither can read back a shape the other has moved on from.
///
