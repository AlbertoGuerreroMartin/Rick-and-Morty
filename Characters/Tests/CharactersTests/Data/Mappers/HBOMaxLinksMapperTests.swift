...
/// This mapper cannot fail; half these tests are the malformed shapes it must survive.
...
struct HBOMaxLinksMapperTests {
    ...
    /// `amazonhbomax` deeplinks into Prime Video; `hbomax` carries dead `urn:hbo:` links.
    ...
    /// Each `max` offer appears twice per episode; whichever arrives first is the answer.
    ...
    /// Marketing URLs carry two UUIDs (show, then episode); the last one is the episode.
